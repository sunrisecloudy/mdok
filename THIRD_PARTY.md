...
`Cargo.lock` is checked in. `scripts/generate_sbom.py` combines locked checksums with offline Cargo metadata to produce a license-aware SPDX 2.3 inventory, and `scripts/package.sh` includes that inventory, deterministic in-toto/SLSA-style provenance, and this notice in a reproducible archive. Dependency vulnerability checks remain part of the release checklist in `mdok-prd/docs/19-release-and-supply-chain.md`.
